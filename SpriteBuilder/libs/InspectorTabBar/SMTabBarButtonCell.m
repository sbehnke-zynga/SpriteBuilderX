...
//    {
        if (self.state == NSOnState) {
            ...
            if (!gradient) {
                NSColor *color1 = [NSColor colorWithCalibratedRed:0.098 green:0.5059 blue:0.9843 alpha:1.0];
                NSColor *color2 = [NSColor colorWithCalibratedRed:0.4275 green:0.702 blue:0.9686 alpha:1.0];
                CGFloat loactions[] = {0.0f, 0.5f, 1.0f};
                gradient = [[NSGradient alloc] initWithColors:@[color1, color1, color2] atLocations:loactions colorSpace:[NSColorSpace genericRGBColorSpace]];
            }
            CGRect croppedFrame = frame;
            croppedFrame.size.height -= 1;
            [gradient drawInRect:croppedFrame angle:-90.0f];
        }
        else {
            self.bezelStyle = NSTexturedRoundedBezelStyle;
        }
